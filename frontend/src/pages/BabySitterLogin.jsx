	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (formData.email && formData.password) {
				...
				toast.success("Babysitter login successful!");
				navigate("/babysitter/dashboard"); // Redirect to babysitter dashboard
			} else {
		...
		<div
			...
			<div className='max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg relative z-10'>
				<div className='text-center'>
					<div className='mx-auto h-16 w-16 rounded-full bg-pink-500 flex items-center justify-center'>
						<FaBabyCarriage className='h-8 w-8 text-white' />
					</div>
				...
				<form className='mt-8 space-y-6' onSubmit={handleSubmit}>
					<div className='rounded-md -space-y-px'>
						<div className='mb-5'>
							...
							<input
								...
								autoComplete='email'
								required
								className='appearance-none rounded relative block w-full px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-pink-500 focus:border-pink-500 focus:z-10 sm:text-sm'
					...
					<div>
						<button
							type='submit'
							disabled={loading}
							className='group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition duration-150 ease-in-out disabled:bg-pink-400 disabled:cursor-not-allowed'>
							{loading ? (
								<>
									<svg
